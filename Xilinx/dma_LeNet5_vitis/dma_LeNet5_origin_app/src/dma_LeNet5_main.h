...
#define FP_IN_FC3_BIAS      "0:/LeNet5/fc3_b.txt"
    ...
    // FRESULT res;           // Function call result
    // TCHAR *Path = "0:/LeNet5";   // Path to the LeNet5 directory
    ...
    // // Open the LeNet5 directory
    // res = f_opendir(&dir, Path);
    // if (res != FR_OK) {
    //     xil_printf("Failed to open directory '%s': %d\n", Path, res);
    // }

    // // Print the file list of the LeNet5 directory
    // xil_printf("File list of directory '%s':\n", Path);
    // while (1) {
    //     res = f_readdir(&dir, &fno);
    //     if (res != FR_OK || fno.fname[0] == 0) {
    //         break;  // Exit if an error occurs or no more files are found
    //     }
    //     xil_printf("%s\n", fno.fname);
    // }
...
//    res = f_open(&fil, FP_IN_CONV1_WEIGHT, FA_READ);
//    if (res != FR_OK) {
